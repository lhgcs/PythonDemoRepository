...
protected:
    void paintEvent(QPaintEvent *){
        ...
        if(txt != ""){
            ...
            painter.drawText(0,0,25,25,Qt::AlignCenter,txt);
        }

        QRect rect((w-img_w)/2,h-25,img_w,25);
        painter.setBrush(QBrush(QColor(239,61,87,0)));
        painter.drawRect(rect);

        font.setPixelSize(18);//改变字体大小
        painter.setFont(font);
        //painter.setPen(QPen(QColor(38,120,108)));
        //painter.setPen(QPen(QColor(209,234,251)));
        painter.setPen(QPen(QColor(18,183,245)));
        if(num <= 0){
            painter.drawText(rect,Qt::AlignCenter,"已售完");
        }else{
