...
int send_packet(int s, int if_index, struct dhcp_msg *msg, int size, unsigned char *mac,
                ...
                uint32_t saddr, uint32_t daddr, uint32_t sport, uint32_t dport)
{
    ...
    destaddr.sll_family = AF_PACKET;
    destaddr.sll_protocol = htons(ETH_P_IP);
    destaddr.sll_ifindex = 1;
    destaddr.sll_halen = ETH_ALEN;
   // memset(destaddr.sll_addr,0xFF,sizeof(destaddr.sll_addr));
	memcpy(destaddr.sll_addr, mac, ETH_ALEN);
	
	return sendto(s, (void *)msg, size, 0, (struct sockaddr*) &destaddr, sizeof(destaddr));
    //return sendmsg(s, &msghdr, 0);
}
*/
...
int receive_packet(int s, struct dhcp_msg *msg)
{
    ...
    if (nread < (int)(sizeof(struct iphdr) + sizeof(struct udphdr))) {
       ...
       // printf("Packet was truncated (read %d, needed %d)", nread, ntohs(packet.ip.tot_len));
       ...
       // printf("UDP dest port (%d) is not DHCP client", ntohs(packet.udp.dest));
