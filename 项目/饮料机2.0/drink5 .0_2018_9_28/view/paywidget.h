...
class PayWidget : public BaseSubWidget
{
    Q_OBJECT
public:
    explicit PayWidget(int w,int h,QStackedWidget *parent = 0);
    ...
    void set_context(int value){
        ...
        int res=TableGoodInfo::getInstence()->table_select_price_stock(App::goodsTableName,value,info);
        if(res){
            stock=info.getStock();
            price=info.getPrice();
            name=info.getName();
...
//                            .arg(QString::number(stock))
//                            .arg(price));

        label_info->setText(QString("<font size=1>商品:%1 &nbsp;价格:%2元 <br/> 货道:%3 &nbsp;库存:%4</font>")
                            .arg(name)
                            .arg(price)
                            .arg(QString::number(value))
                            .arg(QString::number(stock)));
